...
const AdminDashboard = () => {
  ...
  const fetchMetrics = async () => {
    try { const res = await API.get('/api/admin/dashboard'); setMetrics(res.data); } catch (err) { console.error(err); }
  };
  ...
  useEffect(() => { if (activeTab === 'users') fetchUsers(); }, [userFilters, activeTab]);
  useEffect(() => { if (activeTab === 'stores') fetchStores(); }, [storeFilters, activeTab]);
  ...
  const handleCreateStore = async (e) => {
    e.preventDefault(); setMsg({ type: '', text: '' });
    try {
      await API.post('/api/admin/stores', { ...storeForm, owner_id: storeForm.owner_id ? parseInt(storeForm.owner_id) : null });
      setMsg({ type: 'success', text: 'Store added successfully!' });
      setStoreForm({ name: '', email: '', address: '', owner_id: '' });
      fetchMetrics();
    } catch (err) { setMsg({ type: 'error', text: err.response?.data?.message || 'Failed.' }); }
  ...
  return (
    <div style={{ 
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: `2px solid ${isDark ? '#F59E0B' : '#1E293B'}`, paddingBottom: '10px', marginBottom: '24px' }}>
        <h2 style={{ margin: '0', fontSize: 'clamp(1.2rem, 4vw, 2rem)', fontWeight: '800', color: isDark ? '#F59E0B' : '#1E293B' }}>👑 Admin Control Center</h2>
        <button onClick={logout} style={{ padding: '10px 20px', background: isDark ? '#F59E0B' : '#1E293B', color: isDark ? '#000000' : '#FAF7F2', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '700' }}>Logout</button>
      </div>

      <div style={{ margin: '24px 0', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
        {['metrics', 'users', 'stores'].map((tab) => (
          <button key={tab} onClick={() => setActiveTab(tab)} style={{ padding: '12px 20px', background: activeTab === tab ? (isDark ? '#F59E0B' : '#1E293B') : (isDark ? '#121212' : '#FFFFFF'), color: activeTab === tab ? (isDark ? '#000000' : '#FAF7F2') : (isDark ? '#FFFFFF' : '#1E293B'), border: `1px solid ${isDark ? '#333333' : '#E5E7EB'}`, borderRadius: '8px', cursor: 'pointer', fontWeight: '600', flex: '1 1 auto' }}>
            {tab === 'metrics' ? 'Dashboard Overview' : tab === 'users' ? 'Manage Users' : 'Manage Stores'}
          </button>
        ))}
      </div>

      {msg.text && <div style={{ padding: '14px', marginBottom: '24px', borderRadius: '8px', background: isDark ? '#1A1A1A' : '#FFFFFF', borderLeft: `5px solid ${msg.type === 'success' ? '#10B981' : '#EF4444'}` }}>{msg.text}</div>}
      ...
      {activeTab === 'metrics' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 280px), 1fr))', gap: '24px' }}>
          {[
            ...
            <div key={idx} style={{ padding: '32px 24px', background: isDark ? '#121212' : '#FFFFFF', borderRadius: '16px', textAlign: 'center', borderTop: `4px solid ${box.accent}`, border: isDark ? '1px solid #222222' : 'none' }}>
              <h3 style={{ margin: '0', color: isDark ? '#9CA3AF' : '#6B7280', fontSize: '15px' }}>{box.title}</h3>
              <p style={{ fontSize: '42px', fontWeight: '800', margin: '12px 0 0 0', color: isDark ? '#F59E0B' : '#1E293B' }}>{box.val}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {activeTab === 'users' && (
        <div>
          <form onSubmit={handleCreateUser} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px', background: isDark ? '#121212' : '#FFFFFF', padding: '24px', borderRadius: '14px' }}>
            {['name', 'email', 'password', 'address'].map((field) => (
              <input key={field} type={field === 'password' ? 'password' : 'text'} placeholder={field} required value={userForm[field]} onChange={(e) => setUserForm({ ...userForm, [field]: e.target.value })} style={{ padding: '12px', borderRadius: '8px', border: '1px solid #E5E7EB', backgroundColor: isDark ? '#1E1E1E' : '#FAF7F2', color: '#FFFFFF' }} />
            ))}
            <div style={{ display: 'flex', gap: '8px' }}>
              <select value={userForm.role} onChange={(e) => setUserForm({ ...userForm, role: e.target.value })} style={{ padding: '12px', borderRadius: '8px', backgroundColor: isDark ? '#1E1E1E' : '#FAF7F2', color: isDark ? '#FFFFFF' : '#1E293B', flexGrow: '1' }}>
                <option value="owner">Store Owner</option>
                <option value="admin">Admin</option>
              </select>
              <button type="submit" style={{ padding: '12px 20px', background: isDark ? '#F59E0B' : '#1E293B', color: isDark ? '#000000' : '#FAF7F2', border: 'none', borderRadius: '8px', fontWeight: '700' }}>Add</button>
            </div>
          </form>
            ...
            <table style={{ width: '100%', minWidth: '800px', borderCollapse: 'separate', borderSpacing: '0 6px' }}>
              <thead>
                <tr style={{ textAlign: 'left', color: isDark ? '#F59E0B' : '#1E293B' }}>
                  ...
                  <th onClick={() => toggleSort('user', 'name')} style={{ padding: '16px', cursor: 'pointer' }}>Name ↕</th>
                  <th onClick={() => toggleSort('user', 'email')} style={{ padding: '16px', cursor: 'pointer' }}>Email ↕</th>
              ...
              <tbody>
                {users.map((u, idx) => (
                  <tr key={u.id} style={{ backgroundColor: isDark ? '#121212' : (idx % 2 === 0 ? '#FFFFFF' : '#FAF7F2') }}>
                    <td style={{ padding: '16px', color: '#6B7280' }}>#{u.id}</td>
                    <td style={{ padding: '16px', fontWeight: '600' }}>{u.name}</td>
                    <td style={{ padding: '16px' }}>{u.email}</td>
                    <td style={{ padding: '16px' }}><span style={{ padding: '4px 10px', borderRadius: '20px', fontSize: '12px', background: '#E0F2FE', color: '#0369A1' }}>{u.role}</span></td>
                    <td style={{ padding: '16px', color: '#F59E0B', fontWeight: '700' }}>{u.role === 'owner' ? (u.store_rating ? `★ ${Number(u.store_rating).toFixed(1)}` : 'Unrated') : 'N/A'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {activeTab === 'stores' && (
        <div>
          <form onSubmit={handleCreateStore} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px', background: isDark ? '#121212' : '#FFFFFF', padding: '24px', borderRadius: '14px' }}>
            <input type="text" placeholder="Store Name" required value={storeForm.name} onChange={(e) => setStoreForm({ ...storeForm, name: e.target.value })} style={{ padding: '12px', borderRadius: '8px', backgroundColor: isDark ? '#1E1E1E' : '#FAF7F2', color: '#FFFFFF' }} />
            <input type="email" placeholder="Store Email" required value={storeForm.email} onChange={(e) => setStoreForm({ ...storeForm, email: e.target.value })} style={{ padding: '12px', borderRadius: '8px', backgroundColor: isDark ? '#1E1E1E' : '#FAF7F2', color: '#FFFFFF' }} />
            <button type="submit" style={{ padding: '12px 20px', background: isDark ? '#F59E0B' : '#1E293B', color: isDark ? '#000000' : '#FAF7F2', border: 'none', borderRadius: '8px', fontWeight: '700' }}>Add Store</button>
          </form>
          ...
          <div style={{ width: '100%', overflowX: 'auto', marginTop: '30px' }}>
            <table style={{ width: '100%', minWidth: '800px', borderCollapse: 'separate', borderSpacing: '0 6px' }}>
              <thead>
                <tr style={{ textAlign: 'left', color: isDark ? '#F59E0B' : '#1E293B' }}>
                  <th style={{ padding: '16px' }}>Store Name</th>
                  <th style={{ padding: '16px' }}>Email</th>
                  <th style={{ padding: '16px' }}>Overall Avg Rating</th>
                </tr>
              </thead>
              <tbody>
                {stores.map((s, idx) => (
                  <tr key={s.id} style={{ backgroundColor: isDark ? '#121212' : (idx % 2 === 0 ? '#FFFFFF' : '#FAF7F2') }}>
